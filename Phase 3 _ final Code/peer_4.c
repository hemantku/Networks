/* This a peer_4 UDP code for peer-to-peer authentication. Last Modified on :11/5/2013 at 10:14 AM
...
//Declarations for the size and port number
#define SERVERPORT "5438" 
#define MAXBUFLEN 1000
#define PEER_4_PORT "24438"
#define BACKLOG 10
...
int main(void)
{
		...
		if ((rv = getaddrinfo("nunki.usc.edu",NULL, &hints, &clientinfo)) != 0) 
		{
		...
		printf("Peer 4 Phase 1: The dynamic UDP port number:%d and IP address:%s \n",ntohs(myaddr.sin_port),ipstr);
		if ((rv = getaddrinfo("nunki.usc.edu",SERVERPORT, &hints, &serverinfo)) != 0) 
		{
			fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
			return 1;
		}
		...
		if ((rv = getaddrinfo("nunki.usc.edu",PEER_4_PORT, &hints, &servinfo)) != 0) 
		{
			fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
			return 1;
		}
		...
		//printf("peer 4: waiting for connections...\n");
		...
		inet_ntop(their_addr.ss_family,get_in_addr((struct sockaddr *)&their_addr),s, sizeof s);
		//printf("peer 4: got connection from %s\n", s);
		if (!fork()) 
		{ 
			close(sockfd); 
