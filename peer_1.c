/* This a peer_1 UDP code for peer-to-peer authentication. Last Modified on :11/5/2013 at 10:14 AM
...
int main(void)
{
		...
		printf("Peer 1 Phase 1: The dynamic UDP port number:%d and IP address:%s \n",ntohs(myaddr.sin_port),ipstr);
...
printf("%s Phase 3: The %s has static TCP port number %s and IP address %s\n",strtok(filebuffer," "),strtok(filebuffer," "),PEER_1_PORT,ipstr);
		...
		if ((rv = getaddrinfo("nunki.usc.edu",PEER_1_PORT, &hints, &servinfo)) != 0) 
		{
			fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
			return 1;
		}
		...
		//printf("peer 1: waiting for connections...\n");
		...
		inet_ntop(their_addr.ss_family,get_in_addr((struct sockaddr *)&their_addr),s, sizeof s);
		//printf("peer 1: got connection from %s\n", s);
		if (!fork()) 
		{ 
			close(sockfd); 
